...
def best_policy(a, b, gamma, r1, r2, U, rows, cols):
    pi = {}
    actlist_1 = [(-1, 0), (0, 1), (1, 0), (0, -1)]
    actlist_2 = [(-2, 0), (0, 2), (2, 0), (0, -2)]
    for i in range(rows):
        for j in range(cols):
...
walls, terminals, rows, cols, gamma, p_walk, p_run = read_and_process_input("input1.txt")
p = p_walk
p_ = (1 - p_walk) / 2
p2 = p_run
p2_ = (1 - p_run) / 2
...
def iteration_main(epsilon = 0.000001):
...
def to_words(policy, rows, cols):
    chars = {(0, 1): 'Walk Right', (-1, 0): 'Walk Up', (0, -1): 'Walk Left', (1, 0): 'Walk Down', (0, 2): 'Run Right',
             (-2, 0): 'Run Up', (0, -2): 'Run Left', (2, 0): 'Run Down', None: 'Exit'}
