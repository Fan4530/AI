...
#     # -0.3, -0.2
#     # 0.7
#     grids_size = input[0].split(',')
#     rows = int(grids_size[0])
#     cols = int(grids_size[1]) # [1] is the ,
...
#     r_walk, r_run = float(r[0]), float(r[1])
#
#     gamma = float(input[p_line + 2])
#     #matrix = [[0 for i in range(3)] for i in range(3)]
#     walk_grid = [[0 for i in range(cols)] for j in range(rows)]
#     run_grid = [[0 for i in range(cols)] for j in range(rows)]
...
#                 run_grid[i][j] = terminals[(i, j)]
#                 walk_grid[i][j] = terminals[(i, j)]
#             else:
#                 walk_grid[i][j] = r_walk
#                 run_grid[i][j] = r_run
#     # input2 = GridMDP([[-100, -100, -100, +1],
#     #                   [-100, None, -100, -1],
#     #                   [-100, -100, -100, -100]],
#     #                  terminals=[(3, 2), (3, 1)], gamma=0.9, step=2)
#     input_walk = [walk_grid, terminals, gamma, 1, p_walk]
#     input_run = [run_grid, terminals, gamma, 2, p_run]
#
# n = 100000
# a = 100
# c = n / a
# a = zeros(n).reshape(a, c)
...
#
#
# a = array([[-1, None], [1, 0], [0, -1], [0, 1]])
# b = array([[-1, 0], [1, 0], [0, -1], [0, 1]])
# print a
# walk_grid = zeros(3 * 5).reshape(3, 5)
# walk_grid[1][1] = None
# walk_grid += 1
...
#     for j in range(cols):
#         # up   left    down   right
#         a[i][j] = [[-1 + i, 0 + j], [0 + i, 1 + j], [1 + i, 0 + j], [0 + i, -1 + j]]
#
#
#         if i == 0 or (i - 1, j) in walls:
#             a[i][j][0][0] = a[i][j][0][0] + 1
#         if j == cols - 1 or (i, j + 1) in walls:
#             a[i][j][1][1] = a[i][j][1][1] - 1
#
#
#         if i == rows - 1 or (i + 1, j) in walls:
#             a[i][j][2][0] = a[i][j][2][0] - 1
#
#         if j == 0 or (i, j - 1) in walls:
#             a[i][j][3][1] = a[i][j][3][1] + 1
#         print a[i][j]
#         # else:
#         #     a[i][j] = [(-1 + i, 0 + j), (1 + i, 0 + j), (0 + i, -1 + j), (0 + i, 1 + j)]
# b = [[[] for i in range(cols)] for j in range(rows)]
#
# for i in range(rows):
#     for j in range(cols):
#         b[i][j] = [[-2 + i, 0 + j], [0 + i, 2 + j], [2 + i, 0 + j], [0 + i, -2 + j]]
#
#
#         if i <= 1  or (i - 1, 0) in walls or (i - 2, 0) in walls:
#             b[i][j][0][0] = b[i][j][0][0] + 2
#         if j >= cols - 2 or (i, j + 1) in walls or (i, j + 2) in walls:
#             b[i][j][1][1] = b[i][j][1][1] - 2
#
#
#         if i >= rows - 2 or (i + 1, j) in walls or (i + 2, j) in walls:
#             b[i][j][2][0] = b[i][j][2][0] - 2
#         if j <= 1 or (i, j - 1) in walls or (i, j - 2) in walls:
#             b[i][j][3][1] = b[i][j][3][1] + 2
#         # else:
#         #     a[i][j] = [(-1 + i, 0 + j), (1 + i, 0 + j), (0 + i, -1 + j), (0 + i, 1 + j)]
#         print "b"
#         print b[i][j]
print(str(time() - start))
r1 = zeros(rows * cols).reshape(rows, cols) - 0.3
r1[4][2] = 10
r1[2][4] = 5


r2 = zeros(rows * cols).reshape(rows, cols) - 0.2
r2[4][2] = 10
r2[2][4] = 5
